...
public class Program
{
    ...
    public static void ExceptionMachine()
    {
        ...
        foreach (string bound in bounds)
        {
            do
            {
                Console.Write($"Enter the {bound} bound: ");

                string? input = Console.ReadLine();
                string output = ValidateInput(input, bound);

                // Avoid when possible the overhead of Exceptions.
                ...
                else
                {
                    if (bound == "lower")
                    {
                        lowerBound = int.Parse(input!);
                    }
                    else
                    {
                        upperBound = int.Parse(input!);
                    }
                    exit = true;
                }
            } while (!exit);
        ...
        //    {
        //        Console.WriteLine($"Error: {ex1.ParamName}");
        //        Console.WriteLine();
        ...
        //            try
        //            {
        //                Console.Write("Enter a new upper bound (or type 'exit' to exit): ");
        //                string? input = Console.ReadLine();
        ...
        //                if (ValidateInput(input, "upper bound"))
        //                {
        //                    upperBound = int.Parse(input!);
        //                    break;
        //                }
        ...
        static string ValidateInput(string? input, string bound)
        {
            if (string.IsNullOrEmpty(input))
            {
                return $"Invalid input: {bound} bound can't be empty.\n";
            }

            if (!int.TryParse(input, out int output))
            {
                return $"Invalid input: {bound} bound must be an integer.\n";
            }
